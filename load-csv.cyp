...
MERGE (t:Train {id:id, direction:direction, type:type}) ON CREATE SET t.saturday = case when saturday = "false" then null else true end
MERGE (s:Stop {name:name, mile:mile})
MERGE (t)-[:STOPS_AT {hour:hour, minute:minute}]->(s);
